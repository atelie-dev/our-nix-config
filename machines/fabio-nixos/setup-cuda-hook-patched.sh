...
# Patched copy of nixpkgs' setup-cuda-hook.sh with the fix from
# https://github.com/NixOS/nixpkgs/pull/545542
# (cudaPackages.setupCudaHook: include nvcc in CUDAToolkit_ROOT)
# Remove this file once the PR is in our nixpkgs pin.
...
setupCUDAToolkit_ROOT() {
  ...
  # BEGIN FIX from PR #545542
  # CMake 4.3.4 requires bin/nvcc to be inside CUDAToolkit_ROOT; the loop
  # above only collects host-side deps, so nvcc (in nativeBuildInputs) is
  # missed. Append its directory so FindCUDAToolkit can locate it.
  local nvccExe
  if nvccExe="$(type -P nvcc)"; then
    addToSearchPathWithCustomDelimiter ";" CUDAToolkit_ROOT "${nvccExe%/bin/nvcc}"
  fi
  # END FIX from PR #545542
